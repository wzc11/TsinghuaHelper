...
class baseReqHelper(object):
    ...
    def _getFakeId(self, code):
        ...
        print html
        Re = re.compile(r'"fakeid":"(?P<fakeid>[^"]*)","nick_name":"(?P<name>[^"]*)","date_time":\d*,"content":"(?P<code>[^"]*)",')
        fakeId = Re.finditer(html.decode('utf-8'))
        #print [x.groupdict() for x in fakeId]
        for item in fakeId:
            if item.groupdict()['code'] == code:
                print 'ok', item.groupdict()
                return item.groupdict()['fakeid']
            else:
